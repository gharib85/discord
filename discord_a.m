function [qd, theta_m, phi_m] = discord_a(rho, N_theta, N_phi)
% Requires:     Need TrX.m VNent.m dm2cm pauliprod2.m
% Author:       Xiao-Ming Lu (luxiaoming@gmail.com)
% Date:         2011/10/22
%
% Description: Generate the quantum discord  for 4by4
% density matrix, the first qubit is the appratus which is measured. If
% there are more than one optimal measurements, reture the one closest to
% theta=pi/2, phi=0. 
%
% Note: all the logarithm functions are with unit log2
%
% Usage: [qd, theta_m, phi_m] = discord(rho <,N_theta <, N_phi >>)
...
% Return:   qd      the quantum discord
%           theta_m theta where the minimun of the conditional entropy
%           occurs, if there are more than one values, return all 
%           phi_m   simlilar to theta_m
%
...
%find the indices of all the minimum
[k,l] = find(conditional_entropy == min_CE);
theta_m = theta_a(k);
phi_m = phi_a(l);
